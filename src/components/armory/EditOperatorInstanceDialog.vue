...
const color = computed(() =>
  op.value ? getRarityBaseColor(Number(op.value.rarity) || 0) : '#888',
);
...
function getSkillIcon(key) {
  if (key === 'basicAttack')
    return WEAPON_ATTACK_ICON[op.value?.weapon] ?? '/icons/default_icon.webp';
  const slug = props.instance?.operatorSlug;
  const file =
    key === 'battleSkill' ? 'battle.webp' : key === 'comboSkill' ? 'combo.webp' : 'ultimate.webp';
...
function getTalentDescription(groupIdx, level) {
  ...
  if (!slug) return '';
  return (
    getOperatorTalentDescription(slug, talentFlatIndex(groupIdx), level - 1, locale.value) || ''
  );
}
...
function getPotentialInfo(level) {
  ...
  const idx = level - 1;
  const name =
    getOperatorPotentialName(slug, idx, locale.value) || `${t('armory.common.potential')} ${level}`;
...
function promotedLabel() {
  if (!props.instance) return '';
  if (canPromote.value)
    return getOperatorUiLabel(props.instance.promoted ? 'promoted' : 'promote', locale.value);
        ...
        <div class="header">
          ...
          <div class="header-info">
            <div class="name-row">
              ...
              <span class="stars" :class="`header-rarity-${op.rarity}`" :style="{ color }">{{
                '★'.repeat(op.rarity)
              }}</span>
            </div>
            <div class="tags">
              <span class="tag" :style="{ color: elColor, borderColor: elColor }">{{
                getGameElementName(op.element, locale)
              }}</span>
