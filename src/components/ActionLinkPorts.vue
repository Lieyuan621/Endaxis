...
<template>
  <div
    ref="portOverlayRef"
    :class="{
      'action-link-ports': true,
      hidden: !show,
      'is-dragging': isDragging,
      disabled: disabled,
    }"
    @mousemove.stop="onOverlayMouseMove"
    @mouseup.stop="onOverlayMouseUp"
    @mouseleave.stop="onOverlayMouseLeave"
  >
    <div
      v-for="p in ports"
      :key="p.side"
      :ref="el => (portRefs[p.side] = el)"
      :style="{ '--background-color': color }"
      :class="[
        'link-port',
        p.class,
        `port-${p.side}`,
        { active: activePort === p.side },
        { disabled: disabled },
      ]"
      @mousedown.stop.prevent="onPortMouseDown($event, p)"
      @mouseenter.stop="onPortMouseEnter($event, p)"
      @mouseleave.stop="onPortMouseLeave"
      @mouseup.stop="onPortMouseUp"
      :title="$t('connection.dragToConnect')"
    ></div>
  </div>
</template>
