/** Generated by the Endaxis game-data compiler. Do not edit. */
import type { WeaponDefinition } from '../../../../core/game-data/equipmentDefinition';
const definition = {
  slug: 'wpn_lance_0003',
  rarity: 4,
  weaponType: 'polearm',
  baseAttackAtLevelNodes: [34, 100, 169, 238, 307, 341],
  traits: [
    {
      key: 'skill1',
      levelCount: 9,
      modifiers: [
        {
          kind: 'attribute',
          attribute: 'agility',
          operation: 'flat',
          value: [12, 21, 31, 40, 50, 60, 69, 79, 93],
        },
      ],
      actionGraph: { main: { nodes: {} }, macros: {} },
      skillId: 'wpn_attr_agi_low',
    },
    {
      key: 'skill2',
      levelCount: 9,
      modifiers: [
        {
          kind: 'panelStat',
          stat: 'attackPercent',
          value: [0.03, 0.054, 0.078, 0.102, 0.126, 0.15, 0.174, 0.198, 0.234],
        },
      ],
      actionGraph: { main: { nodes: {} }, macros: {} },
      skillId: 'wpn_sp_attr_atk_low',
    },
    {
      key: 'skill3',
      levelCount: 9,
      modifiers: [],
      actionGraph: {
        main: {
          nodes: {
            applyBuff_1: {
              action: {
                kind: 'applyBuff',
                parameters: {
                  buffId: 'buff_wpn_lance_0003',
                  target: 'caster',
                  blackboardAssignments: {
                    atk_up: { kind: 'blackboard', key: 'install_0_atk_up' },
                  },
                },
              },
              next: null,
            },
          },
        },
        macros: {},
      },
      skillId: 'sk_wpn_lance_0003',
      initializationSequence: { $sequence: 'applyBuff_1' },
      blackboard: { install_0_atk_up: [0.15, 0.18, 0.21, 0.24, 0.27, 0.3, 0.33, 0.36, 0.42] },
    },
  ],
  assetSlug: 'wpn_polearm_0003',
  iconPath: '/weapons/polearm/wpn_polearm_0003.webp',
  buffDefinitions: {
    buff_wpn_lance_0003: {
      stackingType: 'unique',
      priority: 0,
      maxStackCount: 0,
      triggerIntervalSeconds: 0,
      waitFirstTriggerInterval: true,
      maxTriggerCount: 1,
      applyTags: [],
      extendTags: [],
      blackboard: { atk_up: 0, hp_ratio_c: 0 },
      attributeModifiers: [
        { attribute: 'Atk', slot: 'baseMultiplier', value: { blackboardKey: 'atk_up' } },
      ],
      actionGraph: { main: { nodes: {} }, macros: {} },
    },
  },
} as const satisfies WeaponDefinition;
export default definition;
