...
import { readCompareType } from '../src/source/nativeEnums.ts';
...
describe('条件共享原生枚举读取', () => {
  ...
  it.each([
    ...
    [
      ...
      { id: 'marker', blackboardKey: '', useBlackboardKey: false, returnTrueIfNotExists: false },
    ],
  ] as const)('%s 使用原生 TargetSource，完整条件 IR 不因编码改变', (name, fields) => {
    expect(
      parse(name, { ...fields, checkTarget: { targetSource: 2, targetGroupKey: 'tar' } }),
    ).toEqual(
      parse(name, { ...fields, checkTarget: { targetSource: 'Context', targetGroupKey: 'tar' } }),
    );
    expect(() =>
      parse(name, { ...fields, checkTarget: { targetSource: 7, targetGroupKey: 'tar' } }),
  ...
  it('实体数量条件保留完整目标引用、保存键及受击目标选项', () => {
    ...
    expect(
      parse('CheckEntityNum', {
        ...fields,
        compareType: 3,
        checkTarget: { ...fields.checkTarget, targetSource: 2 },
      }),
  ...
  it('生命条件的目标来源编码不改动既有比较/阈值语义', () => {
    ...
    expect(
      parse('CheckHp', { ...fields, hpOwner: { targetSource: 2, targetGroupKey: 'tar' } }),
    ).toEqual(
      parse('CheckHp', { ...fields, hpOwner: { targetSource: 'Context', targetGroupKey: 'tar' } }),
    );
  });
});
...
describe('ForceSpellStatus 精确读取 EnergyShardType', () => {
  ...
  it.each([4, 'Enum', 'Burst', 'Physical', '0', -1])(
    '不把标记成员/其他类型当成法术状态：%j',
    value => {
      expect(() =>
        parseForcedElementalStatusActionSource({ ...action, spellStatusType: value }, 'force', {}),
