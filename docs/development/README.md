...
测试和初始化钩子的默认超时为 30 秒，用于发现卡死，不作为性能验收线。完整编译、源码扫描包含磁盘读取，在 CI 上不能沿用 5 秒假定；少数更重的检查可明确声明更长预算并说明原因。出现超时时先检查重复工作、未完成的异步操作和资源竞争，不启用自动重试掩盖失败。性能比较使用同一环境下的耗时报告，不能把不同机器的一次执行时间直接比较。
